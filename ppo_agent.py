...
class PPOAgent(Agent):
    ...
    def train(self,
        ...
        print('Started training at',
              training_start_time.strftime('%d-%m-%Y %H:%M:%S'))
        ...
        for episode in range(1, episodes + 1):
            ...
            while not done:
                ...
                transitions.append(
                    (observation, action, log_prob, reward, new_observation))
                if len(transitions) >= buffer_size:
                    ...
                    old_a_logp = tf.expand_dims(tf.convert_to_tensor(
                        [x[2] for x in transitions]), axis=1)
                    rewards = tf.expand_dims(tf.convert_to_tensor(
                        [x[3] for x in transitions], dtype=np.float32), axis=1)
                    new_states = tf.convert_to_tensor(
                        [x[4] for x in transitions])

                    discounted_rewards = rewards + \
                        gamma * self._model(new_states)[1]
                    ...
                    for _ in range(ppo_epochs):
                        ...
                        for batch in gen_batches(indices, batch_size):
                            ...
                            with tf.GradientTape() as tape:
                                ...
                                a_logp = tf.reduce_sum(dist.log_prob(
                                    tf.gather(actions, batch)), axis=1, keepdims=True)
                                ratio = tf.exp(
                                    a_logp - tf.gather(old_a_logp, batch))
                                surr1 = ratio * tf.gather(adv, batch)
                                surr2 = tf.clip_by_value(
                                    ratio, 1.0 - clip_epsilon, 1.0 + clip_epsilon) * tf.gather(adv, batch)
                            ...
                            self._model.optimizer.apply_gradients(
                                zip(g, self._model.trainable_variables))
            ...
            if not episode % log_interval:
                print(
                    f'Episode {episode} | Reward: {episode_reward:.02f} | Moving Average: {np.average(episode_rewards[-50:]):.02f}')
        ...
        print('Finished training at',
              training_end_time.strftime('%d-%m-%Y %H:%M:%S'))
        ...
        moving_averages = [np.mean(episode_rewards[i - (moving_average_range - 1):i + 1]) if i >= (
            moving_average_range - 1) else np.mean(episode_rewards[:i + 1]) for i in range(len(episode_rewards))]
        plt.plot(x_axis, moving_averages, color='red',
                 label=f'{moving_average_range}-episode moving average')
    ...
    def _create_model(self):
        ...
        def create_conv_layer(filters, kernel_size, strides):
            return layers.Conv2D(filters, kernel_size=kernel_size, strides=strides,
                                 activation='relu',
