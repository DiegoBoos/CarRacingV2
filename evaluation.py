...
def evaluate(env, agent, video_path=None, fps: int = 50, render=True, episodes: int = 1, render_observation_video: bool = False):
    ...
    if render_observation_video and video_path is not None:
        observation_video_writer = cv2.VideoWriter('observation-' + video_path, cv2.VideoWriter_fourcc(
            *'mp4v'), fps, (combined_observation.shape[1], combined_observation.shape[0]))
    ...
    for episode in range(episodes):
        ...
        if video_writer is None and video_path is not None:
            ...
            video_writer = cv2.VideoWriter(video_path, cv2.VideoWriter_fourcc(
                *'mp4v'), fps, (frame.shape[1], frame.shape[0]))
        ...
        while not done:
            ...
            if observation_video_writer is not None:
                combined_observation[:32, :32] = np.interp(
                    observation[..., 0], [-1, 1], [0, 1]) * 255
                combined_observation[:32, 32:] = np.interp(
                    observation[..., 1], [-1, 1], [0, 1]) * 255
                combined_observation[32:, :32] = np.interp(
                    observation[..., 2], [-1, 1], [0, 1]) * 255
                combined_observation[32:, 32:] = np.interp(
                    observation[..., 3], [-1, 1], [0, 1]) * 255
                observation_video_writer.write(cv2.cvtColor(
                    combined_observation, cv2.COLOR_GRAY2BGR))
            ...
            if video_writer is not None:
                ...
                cv2.putText(bgr_frame, f'S: {action[0]:.02f}', (
                    540, 365), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
                cv2.putText(bgr_frame, f'G: {action[1]:.02f}', (
                    540, 380), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
                cv2.putText(bgr_frame, f'B: {action[2]:.02f}', (
                    540, 395), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1, cv2.LINE_AA)
